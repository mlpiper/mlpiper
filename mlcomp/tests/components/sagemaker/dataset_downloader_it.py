...
class DatasetDownloaderIT(ConnectableComponent):
    ...
    def _materialize(self, parent_data_objs, user_data):
        ...
        try:
            ...
            self._logger.info("Dataset downloaded and loaded! " +
                              "#samples in train set: {}, ".format(len(train_set[0]) if train_set else None) +
                              "#samples in valid set: {}, ".format(len(valid_set[0]) if valid_set else None) +
                              "#samples in test set: {}".format(len(test_set[0])) if test_set else None)
    ...
    def _init_params(self):
        ...
        self._valid_set_local_csv_filepath = self._params.get('valid_set_local_csv_filepath')
        self._test_set_local_csv_filepath = self._params.get('test_set_local_csv_filepath')
