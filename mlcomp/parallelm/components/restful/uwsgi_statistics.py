...
class UwsgiStatistics(object):
    ...
    def __init__(self, reporting_interval_sec, target_path, sock_filename, logger):
        self._logger = logger
    ...
    def _report_requests_per_window_time(self):
        ...
        mlops.set_stat(StatsConstants.REQS_PER_WINDOW_TIME_GRAPH_TITLE
                       .format(self._reporting_interval_sec), self._curr_stats_snapshot.total_requests_diff)
    ...
    def _report_acc_requests_and_status(self):
        ...
        mlops.set_stat(tbl)
        mlops.set_stat(PredefinedStats.PREDICTIONS_COUNT, self._curr_stats_snapshot.total_requests_diff)

    def _report_avg_response_time_metrics(self):
        self._logger.debug("Reporting about workers average response time ...")
        tbl = Table().name(StatsConstants.AVG_RESP_TIME_TABLE_NAME).cols([StatsConstants.AVG_RESP_TIME_COL_NAME])
