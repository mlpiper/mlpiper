...
class MLPiper(Base):
    ...
    def prepare_run_component(self, args_filename=None):
        # Expected that user provides a path to one component,
        # so len(self._comp_repo_info) which dictionary by EngineType must be 1.
        if len(self._comp_repo_info) != 1:
            raise Exception("Found len(self._comp_repo_info). Only 1 engine expected")

        # Expected that user provides a path to one component,
        # so len(comps) must be 1.
        comps = list(self._comp_repo_info.values())[0]
        if len(comps) != 1:
            raise Exception("Found {} components in the provided path. Only 1 component is expected".format(len(comps)))

        args = {}
        if args_filename is not None:
            with io.open(args_filename, encoding="utf-8") as f:
                lines = f.readlines()

            lines = [l.strip().split(":") for l in lines]
            for l in lines:
                if len(l) != 2:
                    raise Exception("Wrong record is provided for {} in {}".format(l, args_filename))
                args[l[0]] = l[1]

        comp_info = ComponentInfo(list(comps.values())[0]["comp_desc"])
        pipeline_dict = {
                            "name": "Autogenerated pipeline",
                            "engineType": None,
                            "pipe": [
                                {
                                    "name": "Component",
        ...
        pipeline_dict["engineType"] = comp_info.engine_type
        pipeline_dict["pipe"][0]["type"] = comp_info.name
        pipeline_dict["pipe"][0]["arguments"] = args
        self.pipeline(json.dumps(pipeline_dict))
