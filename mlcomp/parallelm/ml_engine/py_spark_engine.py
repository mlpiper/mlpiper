...
from parallelm.components.spark_pipeline_model_helper import SparkPipelineModelHelper
from parallelm.ml_engine.ml_engine import MLEngine


class PySparkEngine(MLEngine):
    def __init__(self, pipeline_name, run_locally, spark_jars=None):
        super(PySparkEngine, self).__init__(pipeline_name)
    ...
    def _session(self):
        if not self._spark_session:
            raise MLCompException("Spark session is not initialized!")
    ...
    def finalize(self):
        if self._output_model_path:
            ...
            model_helper.set_interim_path_to_use_for_model(tmp_model_path=self._interim_directory_path)
    ...
    def set_output_model_path(self, path):
        if self._output_model_path:
            raise MLCompException("Output model path was already set for the given pipeline! pipeline: {}, "
                                  "existing-path: {}, new-path: {}"
                                  .format(self.name(), self._output_model_path, path))
    ...
    def set_interim_directory(self, interim_path):
        """
        Method is responsible for providing interim directory access for MCenter to temporary save model!
