...
def main(_):
  ...
  for i in range(FLAGS.training_iteration):
    ...
    _, train_summary, train_cost, train_acc = sess.run([train_step, train_stats_op, cross_entropy, accuracy], feed_dict={x: batch[0], y_: batch[1]})
    print("Training. step=", i, "accuracy=", train_acc, "cost=", train_cost)
    tb_writer.add_summary(train_summary, i)
    tb_writer.flush()
  print('Done training!')

  test_summary, test_cost, test_acc = sess.run([test_stats_op, cross_entropy, accuracy], feed_dict={x: mnist.test.images, y_: mnist.test.labels})
