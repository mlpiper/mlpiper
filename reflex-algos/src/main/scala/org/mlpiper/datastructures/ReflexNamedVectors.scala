...
import org.mlpiper.datastructures.ColumnEntryCompareVal._

case class NamedPrediction(labelData: Option[Any],
                           data: Array[ColumnEntry],
                           score: Double,
  ...
  def compare(that: NamedPrediction): Boolean = {
    ...
    NamedVectorUtils.compareColumnEntryArray(this.data, that.data)
  }
}


case class ColumnEntry(var columnName: String, columnValue: Any, columnType: OpType)
  ...
  /**
    ...
    */
  def compare(that: ColumnEntry): ColumnEntryCompareVal = {
...
case class NamedVector(var vector: Array[ColumnEntry],
  ...
  def toContinuousNamedVector(dropNa: Boolean = false): NamedVector = {
    val vectorBuf = scala.collection.mutable.ArrayBuffer.empty[ColumnEntry]
      ...
      if (eachColEntry.columnType.equals(OpType.CONTINUOUS)) {
        try {
          ...
          if (!columnValue.isNaN) {
            vectorBuf += ColumnEntry(columnName = eachColEntry.columnName,
              columnValue,
  ...
  def toCategoricalNamedVector(dropNa: Boolean = false): NamedVector = {
    val vectorBuf = scala.collection.mutable.ArrayBuffer.empty[ColumnEntry]
  ...
  def getColumnEntryFromColumnName(columnName: String): Option[ColumnEntry] = {
    for (columnEntry: ColumnEntry <- vector) {
  ...
  def selectCols(colsName: Set[String]): NamedVector = {
    val vectorBuf = scala.collection.mutable.ArrayBuffer.empty[ColumnEntry]
  ...
  def compare(that: NamedVector): Boolean = {
    ...
    if (label1.isDefined && !NamedVectorUtils.compareAnyValues(label1.get.toString, label2.get.toString)) {
      return false
    }
    ...
    val values1: Array[ColumnEntry] = this.vector
    val values2: Array[ColumnEntry] = that.vector
    NamedVectorUtils.compareColumnEntryArray(values1, values2)
  }
}

object NamedVectorUtils {
  /**
    * Parsing function for NamedVector.
    * Reciprocal of NamedVector's toString.
    *
    * ie. s"($columnsString; $label; $timeStamp)" -> NamedVector(columns, label, timeStamp)
    *
    * @param namedVectorStr String containing NamedVector
    */
  def fromString(namedVectorStr: String): Option[NamedVector] = {
    val splitRegex = "[(),]".r
    val split = splitRegex.split(namedVectorStr).filter(x => x != " " && x != "").map(_.replaceAll(" ", "").replaceAll(";", ""))
    val columnEntryArity = ColumnEntry("", "", OpType.CATEGORICAL).productArity // must use arity of valid column entry type
  ...
  def createColumnEntries(columnNames: Array[String],
                          columnValues: Array[Any]): Array[ColumnEntry] = {
    ...
    val columnEntries: ArrayBuffer[ColumnEntry] = new ArrayBuffer[ColumnEntry]()

    for ((columnName, columnValue) <- columnNames zip columnValues) {
      // considering all will be always continuous
      columnEntries += ColumnEntry(columnName, columnValue, OpType.CONTINUOUS)
    }

    columnEntries.toArray
  }

  def compareReflexPredictionSeq(seq1: Seq[NamedPrediction], seq2: Seq[NamedPrediction]): Boolean = {
  ...
  def compareNamedVectorSeq(seq1: Seq[NamedVector], seq2: Seq[NamedVector]): Boolean = {
    if (seq1.length != seq2.length) return false
    for ((x, y) <- seq1 zip seq2) {
      if (!x.compare(y)) {
        return false
  ...
  def compareColumnEntryArray(arr1: Array[ColumnEntry], arr2: Array[ColumnEntry]): Boolean = {
    ...
    for ((x, y) <- arr1_sorted zip arr2_sorted) {
      ...
      if (!NamedVectorUtils.compareAnyValues(x.columnValue, y.columnValue)) {
        return false
      }
    }
    true
