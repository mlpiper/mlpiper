...
import com.parallelmachines.reflex.common.BarGraphFormat
...
case class HistogramWrapper(histogram: mutable.Map[String, Histogram]) {
  override def toString: String = {
    ParsingUtils.iterableToJSON(histogram.map(x => (x._1, x._2.toGraphJsonable())))
  }
}
...
object HistogramWrapper {
  def fromString(stringRep: String): mutable.Map[String, Histogram] = {
    ...
    for (eachKey <- mapOfSerializedHist.keys) {
      ...
      featuredHist(eachKey) = new Histogram((BarGraphFormat.listMapToLinkedHashMap(serializedHistogram)).toMap)
    }
