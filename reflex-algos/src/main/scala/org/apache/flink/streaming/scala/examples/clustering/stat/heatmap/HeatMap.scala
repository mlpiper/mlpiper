package org.apache.flink.streaming.scala.examples.clustering.stat.heatmap
...
import org.apache.flink.streaming.scala.examples.clustering.stat.heatmap.globalgenerator.{MinMaxHeatMap, MinMaxParamsGenerator, StandardScaleHeatMap, StandardScaleParamsGenerator}
import org.apache.flink.streaming.scala.examples.clustering.stat.heatmap.localgenerator.{MeanHeatMap, NormalizedMeanHeatMap, NormalizedMeanHeatMapHelper}
import org.apache.flink.streaming.scala.examples.clustering.stat.utils.{IDElement, PrependElementID, StreamsCombiner}
...
import org.apache.flink.streaming.scala.examples.functions.map.SubtaskElement
...
object HeatMap {
  ...
  /**
    ...
    */
  def createHeatMap(rddOfNamedVec: RDD[ReflexNamedVector],
  ...
  /**
    * Generate the heatMaps for the input iterable of ReflexNamedVector.
    *
    ...
    * @return A stream of heatmap values calculated on each mini-batches.
    */
  private def generateLocalHeatMaps[LocalW <: Window](streamOfVectors: DataStream[ReflexNamedVector],
                                                      localWindowFunction: KeyedStream[SubtaskElement[ReflexNamedVector], Int]
                                                        => WindowedStream[SubtaskElement[ReflexNamedVector], Int, LocalW],
  ...
  : DataStream[HeatMapValues] = {
    ...
    val localWindowBatch: DataStream[Iterable[ReflexNamedVector]] = SubtaskWindowBatch.batch(inputStream = streamOfVectors, windowFunction = localWindowFunction)

    // calculate heatMap by using flatMap functionality on iterable of denseVectors.
    val heatMapValues: DataStream[HeatMapValues] =
      localWindowBatch
        .flatMap(localHeatMapCalculator
          .asInstanceOf[RichFlatMapFunction[Iterable[ReflexNamedVector], HeatMapValues]])

    heatMapValues
  }

  def generateHeatMapsFromWindows[LocalW <: Window, GlobalW <: Window](stream: DataStream[ReflexNamedVector],
                                                                       localWindowFunction: KeyedStream[SubtaskElement[ReflexNamedVector], Int]
                                                                         => WindowedStream[SubtaskElement[ReflexNamedVector], Int, LocalW],
                                                                       localHeatMapMethod: HeatMapMethod.HeatMapTypes,
  ...
  : KeyedStream[SubtaskElement[ReflexNamedVector], Int] => WindowedStream[SubtaskElement[ReflexNamedVector], Int, GlobalWindow] = {
    require(localWindowSize > 0, "Local window size must be positive number")
    (x: KeyedStream[SubtaskElement[ReflexNamedVector], Int]) => x.countWindow(localWindowSize)
  }

  private def getLocalWindowFunction(localWindowSize: Time)
  : KeyedStream[SubtaskElement[ReflexNamedVector], Int] => WindowedStream[SubtaskElement[ReflexNamedVector], Int, TimeWindow] = {
    (x: KeyedStream[SubtaskElement[ReflexNamedVector], Int]) => x.timeWindow(localWindowSize)
  }
  ...
  /**
    ...
    *
    * @param stream              DataStream of ReflexNamedVector.
    * @param localHeatMapMethod  Method to use to generate heatMap. (i.e. "local-by-mean")
    ...
    */
  def createHeatMap(stream: DataStream[ReflexNamedVector],
                    localHeatMapMethod: HeatMapMethod.HeatMapTypes,
