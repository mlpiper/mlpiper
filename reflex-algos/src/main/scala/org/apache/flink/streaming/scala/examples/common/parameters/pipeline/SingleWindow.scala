...
/**
  * Setter functions for a single time window or count window, but not both.
  * [[SingleTimeWindow]] and [[SingleCountWindow]] both use [[PMWindowMode]] as their
  * [[ArgumentParameter]] key in the [[ArgumentParameterMap]].
  *
  * @tparam Self Class to use these setter methods.
  */
trait SingleWindow[Self]
  extends WithArgumentParameters
    with SingleTimeWindow[Self] with SingleCountWindow[Self] {
  that: Self =>
}
