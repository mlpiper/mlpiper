...
object NamedMatrixToFeaturedHistogram {
  ...
  /**
    * Method/API is responsible for creating Featured Histogram from ReflexNamedMatrix for given bin ranges of each attributes.
    */
  def apply(namedMatrixRep: ReflexNamedMatrix,
...
/**
  * Compute histogram on DataStream of ReflexNamedVector which contains Matrix Representation.
  *
  ...
  *
  * @param binSizeForEachFeatureForRef     Map of bin sizes required for each features
  * @param minBinValueForEachFeatureForRef Map of least values in bin required for each features
  * @param maxBinValueForEachFeatureForRef Map of max values in bin required for each features
  */
class NamedMatrixToFeaturedHistogram(binSizeForEachFeatureForRef: Option[Map[String, Double]] = None,
                                     minBinValueForEachFeatureForRef: Option[Map[String, Double]],
                                     maxBinValueForEachFeatureForRef: Option[Map[String, Double]])
  extends RichMapFunction[ReflexNamedMatrix, mutable.Map[String, Histogram]] {

  override def map(namedMatrixRep: ReflexNamedMatrix)
