...
case object Soft extends PositiveDoubleParameter {
  override val key = "soft"
  override val label: String = "Softness"
  override val required = true
  override val description = "Softness of SGD"
}

trait Soft[Self] extends WithArgumentParameters {

  that: Self =>

  def setSoft(soft: Double): Self = {
    this.parameters.add(Soft, soft)
    that
  }
}
