...
case object StepSize extends PositiveDoubleParameter {
  override val key = "stepSize"
  override val label: String = "Step Size"
  override val required = false
  override val defaultValue: Option[Double] = Some(1.0)
  override val description = "Step Size of SGD. (Default: 1.0)"
}

trait StepSize[Self] extends WithArgumentParameters {

  that: Self =>

  def setStepSize(stepSize: Double): Self = {
    this.parameters.add(StepSize, stepSize)
    that
  }
}
