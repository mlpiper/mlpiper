...
class GlobalAccumulator[R](name: String,
  ...
  */
  ...
  private val flinkAccumulator = new GlobalStatFlinkAccumulator[R](globalMerge)
  ...
  /**
    * Registers the Flink accumulator to the RuntimeContext if it has not been registered. Uses
    * the localStat name. Replaces the accumulator values with the GlobalStat's count and localValue.
    *
    * @param ctx Flink RuntimeContext
    */
  final def updateFlinkAccumulator(ctx: RuntimeContext): Unit = {
    if (!flinkAccumulator.isRegisteredAccumulator) {
      ctx.addAccumulator(name, flinkAccumulator)
      flinkAccumulator.registerAccumulator()
    }
    flinkAccumulator.add(localInfo)
  }
...
/**
  * Internal FlinkAccumulator for a GlobalStat. Uses [[AccumulatorInfo]] as input to hold a local
  * and count. Uses [[AccumulatorInfo]] as output to wrap the input type within a Serializable class.
  *
  * @tparam R Type of the StatAccumulator.
  */
sealed class GlobalStatFlinkAccumulator[R](override protected val globalMerge: MergeableStat[R])
  extends Accumulator[AccumulatorInfo[R], AccumulatorInfo[R]] with StatAccumulator[R] {

  override def resetLocal(): Unit = {}

  override def getLocalValue: AccumulatorInfo[R] = this.get

  override def merge(otherAccum: Accumulator[AccumulatorInfo[R], AccumulatorInfo[R]]): Unit = {
