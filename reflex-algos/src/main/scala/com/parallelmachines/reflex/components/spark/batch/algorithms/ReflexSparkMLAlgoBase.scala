...
trait ReflexSparkMLAlgoBase extends SparkBatchComponent with ModelBehavior {
  ...
  var sparkSaveModel: SparkSaveModel = new SparkSaveModel()
  ...
  final override def materialize(env: SparkContext, dsArr: ArrayBuffer[DataWrapperBase],
    ...
    sparkSaveModel.setSparkMLModel(pipelineModel)
    sparkSaveModel.setTempSharedPathStr(tempSharedPathStr)

    ArrayBuffer(DataWrapper(sparkSaveModel),
