...
import org.apache.flink.streaming.scala.examples.clustering.stat.categorical.{CombineFeaturedHistograms => CombineCategoricalFeaturedHistograms, HistogramFormatting => CategoricalHistogramFormatting, HistogramWrapper => CategoricalHistogramWrapper, NamedMatrixToFeaturedHistogram => NamedMatrixToCategoricalFeaturedHistogram}
...
object SparkMLPredictionDistributionClassificationStat extends Serializable {
        ...
        globalMerge = (x: AccumulatorInfo[CategoricalHistogramWrapper],
          ...
          AccumulatorInfo(
            ...
            value = CategoricalHistogramWrapper(
              CombineCategoricalFeaturedHistograms.combineTwoFeaturedHistograms(x.value.histogram, y.value.histogram)),
  ...
  def createStat(predictionRDD: RDD[Any], predictionColName: String, sc: SparkContext): Unit = {
    val namedPredictionRDD = predictionRDD.map(eachPredictedLabel => {
      ...
      ReflexNamedVector(vector = Array(rce))
    })
    ...
    if (!rddOfNamedMatrix.isEmpty()) {
      ...
      val histogram = rddOfNamedMatrix
        ...
        .reduce((x, y) => CombineCategoricalFeaturedHistograms.combineTwoFeaturedHistograms(x, y))
