...
import com.parallelmachines.reflex.common.dataanalysis.CategoricalDataAnalyst
...
import org.apache.flink.streaming.scala.examples.clustering.stat.categorical.{HistogramFormatting, Histogram => CategoricalHistogram, HistogramWrapper => CategoricalHistogramWrapper}
...
class CategoricalHistogramForSpark(healthName: String) extends HealthComponentSpark {
  ...
  def createHealth(): Option[mutable.Map[String, CategoricalHistogram]] = {
    ...
    val genericHealthForCategoricalFeatures = new HistogramForCategoricalFeatures(healthName)

    val rddOfNamedVector: RDD[ReflexNamedVector] = if (rddOfDenseVector != null) {
  ...
  var contenderHistogram: mutable.Map[String, CategoricalHistogram] = _
  ...
  def createHealthAndCompare(): Unit = {
    ...
    if (inferenceHist.isDefined) {
      ...
      val genericHistogramForCategoricalFeatures = new HistogramForCategoricalFeatures(accumName = this.healthName)
