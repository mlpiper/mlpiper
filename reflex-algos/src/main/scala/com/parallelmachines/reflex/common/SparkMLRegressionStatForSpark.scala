...
import org.apache.flink.streaming.scala.examples.clustering.stat.continuous.{CombineFeaturedHistograms => CombineContinuousFeaturedHistograms, HistogramWrapper => ContinuousHistogramWrapper, NamedMatrixToFeaturedHistogram => NamedMatrixToContinuousFeaturedHistogram}
        ...
        globalMerge = (x: AccumulatorInfo[ContinuousHistogramWrapper],
          ...
          AccumulatorInfo(
            ...
            value = ContinuousHistogramWrapper(
              CombineContinuousFeaturedHistograms.combineTwoFeaturedHistograms(x.value.histogram, y.value.histogram)),
  ...
  def createStat(predictionRDD: RDD[Any], predictionColName: String, sc: SparkContext): Unit = {
    ...
    if (!rddOfNamedMatrix.isEmpty()) {
      ...
      val histogram = rddOfNamedMatrix
        ...
        .reduce((x, y) => CombineContinuousFeaturedHistograms.combineTwoFeaturedHistograms(x, y))
