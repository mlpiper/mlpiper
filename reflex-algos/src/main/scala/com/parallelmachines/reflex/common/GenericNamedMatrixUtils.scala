...
import org.apache.flink.streaming.scala.examples.clustering.math.{ReflexColumnVectorEntry, ReflexNamedMatrix, ReflexNamedVector}
...
object GenericNamedMatrixUtils {
  ...
  def iteratorOfNamedVectorToNamedMatrix(iteratorOfNamedVector: Iterator[ReflexNamedVector]): ReflexNamedMatrix = {
    ...
    val reflexColumnVectorEntry: Array[ReflexColumnVectorEntry] = new Array[ReflexColumnVectorEntry](totalCols)
    ...
    columnValues.keys.zipWithIndex.foreach(eachColAndIndex => {
      reflexColumnVectorEntry(eachColAndIndex._2) =
        ReflexColumnVectorEntry(columnName = eachColAndIndex._1,
          columnValue = columnDensedValues(eachColAndIndex._1))
    })

    ReflexNamedMatrix(arrayOfVector = reflexColumnVectorEntry)
  }
  ...
  : ReflexNamedMatrix = {
    ...
    val reflexColumnEntry: Array[ReflexColumnVectorEntry] = new Array[ReflexColumnVectorEntry](transpose.rows)
    ...
    for (eachFeature <- 0 until transpose.rows) {
      ...
      reflexColumnEntry(eachFeature) = ReflexColumnVectorEntry(columnName = namesOfFeatures(eachFeature), columnValue = innerVector)
    }

    ReflexNamedMatrix(arrayOfVector = reflexColumnEntry)
  }
  ...
  def createReflexNamedMatrix(rddOfNamedVector: RDD[ReflexNamedVector])
  : RDD[ReflexNamedMatrix] = {
  ...
  def createReflexNamedMatrix(dataStreamOfNamedVector: DataStream[ReflexNamedVector], windowingSize: Long)
  : DataStream[ReflexNamedMatrix] = {
