...
import StringToIndexedVectorTestData._
...
class StringToIndexedBreezeVectorITCase extends StreamingMultipleProgramsTestBase with Matchers {
  ...
  @Test def testAllIndicesCommaSeparated(): Unit = {
    ...
    val flatMap = new StringToIndexedBreezeVectorFlatMap(validVectorNrElements, ",", indices)
    val output = DataStreamUtils(textStream.flatMap(flatMap)).collect().toSeq

    compareBreezeOutputStream(output, validVectors, indices) should be(true)
  }
  ...
  @Test def testInvalidVectorsCommaSeparated(): Unit = {
    ...
    val flatMap = new StringToIndexedBreezeVectorFlatMap(invalidVectorNrElements, ",", indices)
    val output = DataStreamUtils(textStream.flatMap(flatMap)).collect().toSeq
  ...
  @Test def testAllIndicesSpaceSeparated(): Unit = {
    ...
    val flatMap = new StringToIndexedBreezeVectorFlatMap(validVectorNrElements, "\\s+", indices)
    val output = DataStreamUtils(textStream.flatMap(flatMap)).collect().toSeq
