...
class ReflexPipelineTest extends FlatSpec with Matchers {
  ...
  "Tensorflow single component pipeline" should "be valid" in {
    val json1 =
      s"""
         |{
         |  "name": "tt",
    ...
    logger.debug(s"cmdline: [$cmdline]")
    val sysConfigParamPattern = "statsDBHost".r
    val match1 = sysConfigParamPattern.findFirstIn(cmdline)
    assert(!match1.isDefined, "Found statsDBHost argument in TF component command line")

    val normalParamPatter = "save_dir".r
    val match2 = normalParamPatter.findFirstIn(cmdline)
    assert(match2.isDefined, "Could not find save_dir in cmdline")

    // Checking programPath
    val progPath = tfComp.programPath()
    assert(Files.exists(Paths.get(progPath)), s"programfile: $progPath does not exists")

    // Checking modelDirArg
    val modelDirArg = tfComp.modelDirArgument()
    assert(modelDirArg.isDefined && modelDirArg.get == "save_dir")

    // Checking isModelProduer
    val isModelProducer = tfComp.isModelProducer
    assert(isModelProducer, "TF component should be a model producer")

    // Checking modelOutPath set function
    tfComp.modelOutputPath("blabla")
    val cmdline2 = tfComp.buildCommandLine()
    logger.info(s"cmdline2: [$cmdline2]")

    val modelOutputPattern = """.*\s+\-\-save_dir\s+(\w+).*""".r

    val path = cmdline2 match {
      case modelOutputPattern(modelDir) => modelDir
      case _ => "not-found"
    }
    assert(path.equals("7"), s"Model output argument content is not correct got: [$path] expected: [7]")

    // Checking componentAdditionalFiles
    var findRes = tfComp.componentAdditionalFiles.find(_ == "mnist_input_data.py")
    assert(!findRes.isEmpty, "mnist_input_data.py should be in the tf additional files")

    findRes = tfComp.componentAdditionalFiles.find(_.endsWith("cnn_mnist.py"))
    assert(findRes.isEmpty, "cnn_mnist.py should NOT be in the tf additional files")

  }

  "Tensorflow single component no save_dir" should "be valid" in {
    val json1 =
      s"""
         |{
         |  "name": "tt",
    ...
    // Checking modelOutPath set function
    val outputValue = "another_value"
    tfComp.modelOutputPath(outputValue)
    val cmdline2 = tfComp.buildCommandLine()
    logger.info(s"cmdline2: [$cmdline2]")
    ...
    assert(path.equals(outputValue), s"Model output argument content is not correct got: [$path] expected: [$outputValue]")
  }
