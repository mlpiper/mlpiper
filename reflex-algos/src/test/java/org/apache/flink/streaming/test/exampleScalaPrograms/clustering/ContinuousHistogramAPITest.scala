...
class ContinuousHistogramAPITest extends FlatSpec with Matchers {
  ...
  /**
    * Testing combine functionality of dataSet of DenseMatrix
    */
  it should "Generate Correct Featured Histogram From Input DenseMatrix" in {
    ...
    val sequenceOfMapOfFeatureIDAndHist: Seq[mutable.Map[String, Histogram]] = Seq[mutable.Map[String, Histogram]](map1OfFeatureIDAndHist, map2OfFeatureIDAndHist)

    // dataSet of map containing featureID and hist
    val dataSetOfMapOfFeatureIDAndHist = env.fromCollection(sequenceOfMapOfFeatureIDAndHist)

    // combining using reduce function
    val combinedFeatureAndHist = dataSetOfMapOfFeatureIDAndHist.reduce(new CombineFeaturedHistograms).collect().head

    val expectedCombinedHistDetailForFeature1 = DenseVector[Double](15.0, 26.0, 24.0)
    val expectedCombinedHistBinForFeature1 = DenseVector[Double](1.0, 3.0, 8.0, 11.0)

    combinedFeatureAndHist(featureID1).hist.equals(expectedCombinedHistDetailForFeature1) should be(true)
    combinedFeatureAndHist(featureID1).binEdges.equals(expectedCombinedHistBinForFeature1) should be(true)

    val expectedCombinedHistDetailForFeature2 = DenseVector[Double](77.0, 124.0)
    val expectedCombinedHistBinForFeature2 = DenseVector[Double](4.0, 7.0, 10.0)

    combinedFeatureAndHist(featureID2).hist.equals(expectedCombinedHistDetailForFeature2) should be(true)
    combinedFeatureAndHist(featureID2).binEdges.equals(expectedCombinedHistBinForFeature2) should be(true)
  }
